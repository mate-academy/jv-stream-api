...
public class JavaStreamApi {
    ...
    public Integer oddSum(List<Integer> numbers) {
        return numbers.stream().mapToInt(x -> x).filter(x -> x % 2 != 0).sum();
    }
    ...
    public String findElement(List<String> elements, String element) {
        return elements.stream().filter(x -> x.equals(element)).findAny()
    ...
    public Double averageSumOdd(List<Integer> numbers) {
        return IntStream.range(0, numbers.size()).map(x -> (x % 2 == 1) ? numbers.get(x) - 1
                : numbers.get(x)).filter(number -> number % 2 == 1)
    ...
    public List<People> manSelectByAge(List<People> peopleList, int fromAge, int toAge) {
        return peopleList.stream().filter(s -> s.getSex() == People.Sex.MAN && s.getAge() >= fromAge
                && s.getAge() <= toAge).collect(Collectors.toList());
    }
    ...
    public List<People> workablePeople(int fromAge, int femaleToAge,
        ...
        return peopleList.stream().filter(s -> s.getSex() == People.Sex.MAN
                && s.getAge() <= maleToAge && s.getAge() >= fromAge
                || (s.getSex() == People.Sex.WOMEN && s.getAge() <= femaleToAge
                && s.getAge() >= fromAge)).collect(Collectors.toList());
    }
    ...
    public List<String> getCatsNames(List<People> peopleList, int femaleAge) {
        return peopleList.stream().filter(s -> s.getSex() == People.Sex.WOMEN
                && s.getAge() >= femaleAge)
                .map(People::getCatList).flatMap(Collection::stream)
                .map(Cat::getName)

...
public class JavaStreamApi {
    ...
    public Double averageSumOdd(List<Integer> numbers) {
        return IntStream
                ...
                .map(ind -> (ind % 2 == 1) ? numbers.get(ind) - 1 : numbers.get(ind))
    ...
    public List<People> manSelectByAge(List<People> peopleList, int fromAge, int toAge) {
        return peopleList
                ...
                && men.getAge() >= fromAge && men.getAge() <= toAge)
    ...
    public List<People> workablePeople(int fromAge, int femaleToAge,
        ...
        return peopleList
                .stream()
                .filter(workers -> workers.getAge() >= fromAge
                        && (workers.getSex() == People.Sex.MAN
                        && workers.getAge() <= maleToAge)
                        || (workers.getSex() == People.Sex.WOMEN
                        && workers.getAge() <= femaleToAge))
                .collect(Collectors.toList());
    }

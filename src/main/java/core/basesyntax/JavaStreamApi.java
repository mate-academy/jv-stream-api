...
public class JavaStreamApi {
    ...
    public Long elementCount(List<String> elements, String element) {
        return elements.stream().filter(x -> x.equals(element)).collect(Collectors.counting());
    }
    ...
    public String findElement(List<String> elements, String element) {
        return elements.stream().filter(x -> x.equals(element)).findFirst().get();
    }
    ...
    public Double averageSumOdd(List<Integer> numbers) {
        return IntStream.range(0, numbers.size())
                ...
                .filter(x -> x % 2 != 0).average().orElseThrow(NoSuchElementException::new);
    }
    ...
    public List<People> manSelectByAge(List<People> peopleList, int fromAge, int toAge) {
        return peopleList.stream().filter(x -> x.getAge() > fromAge && x.getAge() < toAge)
                .filter(x -> x.getSex().equals(People.Sex.MAN)).collect(Collectors.toList());
    }
    ...
    public List<People> workablePeople(int fromAge, int femaleToAge,
        ...
        return peopleList.stream().filter(x -> (x.getSex().equals(People.Sex.MAN)
                && x.getAge() > fromAge && x.getAge() < maleToAge)
                || (x.getSex().equals(People.Sex.WOMEN) && x.getAge() > fromAge

...
public class JavaStreamApi {
    ...
    public List<People> manSelectByAge(List<People> peopleList, int fromAge, int toAge) {
        return peopleList.stream()
                .filter(youAreInTheArmyNow -> youAreInTheArmyNow.getAge() >= fromAge
                && youAreInTheArmyNow.getAge() <= toAge
                && youAreInTheArmyNow.getSex().equals(People.Sex.MAN))
    ...
    public List<People> workablePeople(int fromAge, int femaleToAge,
        ...
        return peopleList.stream()
                .filter(worker -> (worker.getAge() >= fromAge
                        && worker.getAge() <= femaleToAge
                        && worker.getSex().equals(People.Sex.WOMEN))
                        || (worker.getAge() >= fromAge
                        && worker.getAge() <= maleToAge
                        && worker.getSex().equals(People.Sex.MAN)))
                .collect(Collectors.toList());

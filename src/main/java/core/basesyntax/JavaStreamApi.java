...
import core.basesyntax.People.Sex;
...
public class JavaStreamApi {
     ...
     * (пример: Arrays.asList(«a1», «a2», «a3», «a1»)). Вернуть количество вхождений объекта
     * `element`</p>
     **/
     ...
     * (пример: Arrays.asList(«a1», «a2», «a3», «a1»)). Вернуть Optional первого элемента
     * коллекции</p>
     **/
     ...
     * (приметр: Arrays.asList(«a1», «a2», «a3», «a1»)). Найти элемент в коллекции равный `element`
     ...
     * Отнимите от каждого элемента, который стоит на непарной позиции (имеет не парный индекс) 1 и
     * верните среднее арифметическое всех нечетных чисел или киньте ошибку
     ...
     * вида Arrays.asList( new People(«Вася», 16, Sex.MAN), new People(«Петя», 23, Sex.MAN), new
     * People(«Елена», 42, Sex.WOMEN), new People(«Иван Иванович», 69, Sex.MAN)). Задача: Выбрать
     * мужчин-военнообязанных (от `fromAge` до `toAge` лет)</p>
     **/
     ...
     * <p>6. Дана коллекция класс People (с полями name — имя, age — возраст, sex — пол),
     * вида Arrays.asList( new People(«Вася», 16, Sex.MAN), new People(«Петя», 23, Sex.MAN), new
     * People(«Елена», 42, Sex.WOMEN), new People(«Иван Иванович», 69, Sex.MAN)). Задача: Найти всех
     * потенциально работоспособных людей в выборке от fromAge до femaleToAge для женщин, и от
     * fromAge до maleToAge для мужчин Пример: от 18 лет и учитывая что женщины выходят в 55 лет, а
     * мужчина в 60</p>
     **/
     ...
     * sex — пол, List of Cats -  кошки этого человека). Дано класс Cat (name - имя кошки, age -
     * возраст кошки). Задача: вивести все имена кошек в которых хозяева это девушки старше 18
     * лет</p>
     **/
    public List<String> getCatsNames(List<People> peopleList, int femaleAge) {
        return peopleList.stream()
            .filter(people -> people.getSex() == Sex.WOMEN

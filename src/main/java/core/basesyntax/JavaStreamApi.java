...
public class JavaStreamApi {
    ...
    public Integer oddSum(List<Integer> numbers) {
        return numbers.stream().mapToInt(x -> x % 2 != 0 ? x : 0).sum();
    }
    ...
    public Long elementCount(List<String> elements, String element) {
        return elements.stream().filter(x -> element.equals(x)).count();
    }
    ...
    public String findElement(List<String> elements, String element) {
        return elements.stream().filter(x -> element.equals(x)).findFirst()
    ...
    public Double averageSumOdd(List<Integer> numbers) {
        return IntStream.range(0, numbers.size())
                ...
                .filter(y -> y % 2 != 0).average().orElseThrow(NoSuchElementException::new);
    }
